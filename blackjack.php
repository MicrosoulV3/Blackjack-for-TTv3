...
if ($CURUSER["class"] < "1") {
    show_error_msg("Sorry...", "This is stupid and anyone can play", 1);
}

if (!function_exists('get_user_name')) {
    function get_user_name($userid)
    {
        $dbquery = SQL_Query_exec("select username from users where id=$userid");
        $dbqueryreturn = mysqli_fetch_array($dbquery);
        return "$dbqueryreturn[username]";
    }
}
// Bet size
...
$dbquery = SQL_Query_exec("SELECT bjwins, bjlosses FROM users WHERE id=$CURUSER[id]");
$dbqueryreturn = mysqli_fetch_array($dbquery);
$tot_wins = $dbqueryreturn['bjwins'];
$tot_losses = $dbqueryreturn['bjlosses'];
...
if ($_POST["game"]) {
    ...
    if ($_POST["game"] == 'start') {
        ...
        if ($points == 21) {
            ...
            if ($waitarr[0] > 0) {
                $dbquery = SQL_Query_exec("select * from blackjack where status='waiting' and userid!=$CURUSER[id] order by date asc LIMIT 1");
                $dbqueryreturn = mysqli_fetch_assoc($dbquery);
                if ($dbqueryreturn["points"] != 21) {
                    $winorlose = "you won " . mksize($mb);
                    SQL_Query_exec("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$CURUSER[id]");
                    SQL_Query_exec("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$dbqueryreturn[userid]");
                    SQL_Query_exec("delete from blackjack where userid=$CURUSER[id]");
                    SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                    $dt = sqlesc(get_date_time());
                    $msg = sqlesc("You lost to $CURUSER[username] (you got $dbqueryreturn[points] points, and $CURUSER[username] got 21 points).\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]Play again![/url]");
                    ...
                    SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                    $dt = sqlesc(get_date_time());
                    $msg = sqlesc("You tied with $CURUSER[username] (Both of you had $dbqueryreturn[points] points).\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]Play again![/url]");
                    SQL_Query_exec("INSERT INTO messages (subject, sender, receiver, added, msg, poster) VALUES('Blackjack game', 0, $dbqueryreturn[userid], $dt, $msg, 0)");

                }
                print("<tr><td align=center>" . T_("YOUR_OPPONENT_WAS") . " " . get_user_name($dbqueryreturn["userid"]) . ", " . T_("THEY_HAD") . " $dbqueryreturn[points] points, $winorlose.<br /><br /><center><b><a href=blackjack.php>" . T_("PLAY_AGAIN") . "</a></b></center></td></tr>");
            } else {
            ...
            if ($waitarr[0] > 0) {
                ...
                if ($dbqueryreturn["points"] > 21) {
                    ...
                    SQL_Query_exec("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$dbqueryreturn[userid]");
                    SQL_Query_exec("delete from blackjack where userid=$CURUSER[id]");
                    SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                    $dt = sqlesc(get_date_time());
                    $msg = sqlesc("You beat $CURUSER[username] (You had $dbqueryreturn[points] points, $CURUSER[username] had $points points).\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]" . T_("BACK") . "[/url]");
                    SQL_Query_exec("INSERT INTO messages (subject, sender, receiver, added, msg, poster) VALUES('Blackjack game', 0, $dbqueryreturn[userid], $dt, $msg, 0)");
                }
                print("<tr><td align=center>Your opponent was " . get_user_name($dbqueryreturn["userid"]) . ", They had $dbqueryreturn[points] points, $winorlose.<br /><br /><center><b><a href=blackjack.php>Play again</a></b></center></td></tr>");
            } else {
                SQL_Query_exec("update blackjack set status = 'waiting', date='" . get_date_time() . "' where userid = $CURUSER[id]");
                print("<tr><td align=center>" . T_("ODTHER_PLAYERS1") . "<br />" . T_("ODTHER_PLAYERS2") . "<br /><br /><center><b><a href=blackjack.php>" . T_("BACK") . "</a></b><br /></center></td></tr>");
            }
        ...
        if ($waitarr[0] > 0) {
            $dbquery = SQL_Query_exec("select * from blackjack where status='waiting' and userid!=$CURUSER[id] order by date asc LIMIT 1");
            $dbqueryreturn = mysqli_fetch_assoc($dbquery);
            if ($dbqueryreturn["points"] == $playerarr['points']) {
                $winorlose = "nobody won";
                SQL_Query_exec("delete from blackjack where userid=$CURUSER[id]");
                SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                $dt = sqlesc(get_date_time());
                $msg = sqlesc("Your opponent was $CURUSER[username], you both had $dbqueryreturn[points] points - it was a tie.\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]Play Again[/url]");
                SQL_Query_exec("INSERT INTO messages (subject, sender, receiver, added, msg, poster) VALUES('Blackjack game', 0, $dbqueryreturn[userid], $dt, $msg, 0)");
            } elseif ($dbqueryreturn["points"] < $playerarr['points'] && $dbqueryreturn['points'] < 21) {
                $winorlose = "you won " . mksize($mb);
                SQL_Query_exec("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$CURUSER[id]");
                SQL_Query_exec("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$dbqueryreturn[userid]");
                SQL_Query_exec("delete from blackjack where userid=$CURUSER[id]");
                SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                $dt = sqlesc(get_date_time());
                $msg = sqlesc("You lost to $CURUSER[username] (You had $dbqueryreturn[points] points, $CURUSER[username] had $playerarr[points] points).\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]Play Again[/url]");
                SQL_Query_exec("INSERT INTO messages (subject, sender, receiver, added, msg, poster) VALUES('Blackjack game', 0, $dbqueryreturn[userid], $dt, $msg, 0)");
            } elseif ($dbqueryreturn["points"] > $playerarr['points'] && $dbqueryreturn['points'] < 21) {
                $winorlose = "you lost " . mksize($mb);
                SQL_Query_exec("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$CURUSER[id]");
                ...
                SQL_Query_exec("delete from blackjack where userid=$dbqueryreturn[userid]");
                $dt = sqlesc(get_date_time());
                $msg = sqlesc("You beat $CURUSER[username] (You had $dbqueryreturn[points] points, $CURUSER[username] had $playerarr[points] points).\n\n [url=" . $site_config["SITEURL"] . "/blackjack.php]Play Again[/url]");
                SQL_Query_exec("INSERT INTO messages (subject, sender, receiver, added, msg, poster) VALUES('Blackjack game', 0, $dbqueryreturn[userid], $dt, $msg, 0)");
            } elseif ($dbqueryreturn["points"] == 21) {
                $winorlose = "you lost " . mksize($mb);
                SQL_Query_exec("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$CURUSER[id]");
                SQL_Query_exec("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$dbqueryreturn[userid]");
                SQL_Query_exec("delete from blackjack where userid=$CURUSER[id]");
...
onclick=window.open('info_blackjack.html','wclose','width=820','height=864','toolbar=yes','status=no','left=30','top=20')><img border='0' src='images/INFO_UPLOAD.png' width='40' height='30' alt='Click to see how to play' title='Click to see how to play' /></a><br /><br />\n");
